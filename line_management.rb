...
class Management
	...
	def get_available_stickers() # return Array of LineStickerSet
		...
		stickers = []
		(0..20).each do |i|
			stickers << LineGui::LineSticker.new(3897, 1, i)
		end
		
		stickersets << LineGui::LineStickerSet(3897, 1, stickers)
	end
	...
	def send_message(to, text, sticker = nil, image = nil)
		...
		add_message(LineGui::LineMessage.new(get_own_user_id(), to, message_id, Time.now.to_i, text, sticker, image))
	end
end
